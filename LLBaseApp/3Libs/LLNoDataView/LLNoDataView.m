...
- (instancetype)initReloadBtnWithFrame:(CGRect)frame LLNoDataViewType:(LLNoDataViewType)type description:(NSString *)description reloadBtnTitle:(NSString *)title
{
    ...
    if (self) {
        ...
        if (type == LLNoInternet) {
            iconImage = LLImageWithNamed(LLNoInternetImageName.length>0?LLNoInternetImageName:LLNoDataSrcName(@"network_xinhao_"));
        }
        
        if (type == LLNoData) {
            iconImage = LLImageWithNamed(LLNoDataImageName.length>0?LLNoDataImageName:LLNoDataSrcName(@"no_data_katong_"));
        }
